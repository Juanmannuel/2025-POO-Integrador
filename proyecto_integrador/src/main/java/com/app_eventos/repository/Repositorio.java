        ...
        return tx(em -> {
            ...
            if (nuevas != null) {
                for (Pelicula p : nuevas) {
                    if (p != null && p.getIdPelicula() != null) nuevasIds.add(p.getIdPelicula());
                }
            }

            // IDs actuales en la join table
            @SuppressWarnings("unchecked")
            List<Object> rows = em.createNativeQuery(
                "select pelicula_id from ciclo_cine_peliculas where ciclo_id = :id")
                .setParameter("id", idCiclo)
                .getResultList();

            Set<Long> actualesIds = new HashSet<>();
            for (Object o : rows) {
                if (o instanceof Number n) actualesIds.add(n.longValue());
                else actualesIds.add(Long.parseLong(o.toString()));
            }

            // Calcular diferencias
            Set<Long> aEliminar = new HashSet<>(actualesIds);
            aEliminar.removeAll(nuevasIds); // solo las destildadas

            Set<Long> aAgregar = new HashSet<>(nuevasIds);
            aAgregar.removeAll(actualesIds); // solo las nuevas tildadas

            // DELETE por cada id a eliminar (cantidad chica)
            for (Long idPel : aEliminar) {
                em.createNativeQuery(
                    "delete from ciclo_cine_peliculas where ciclo_id = :cid and pelicula_id = :pid")
                  .setParameter("cid", idCiclo)
                  .setParameter("pid", idPel)
                  .executeUpdate();
            }

            // INSERT por cada id a agregar
            for (Long idPel : aAgregar) {
                em.createNativeQuery(
                    "insert into ciclo_cine_peliculas (ciclo_id, pelicula_id) values (:cid, :pid)")
                  .setParameter("cid", idCiclo)
                  .setParameter("pid", idPel)
                  .executeUpdate();
            }

            // Devolver el ciclo con su colección refrescada
            em.flush();
