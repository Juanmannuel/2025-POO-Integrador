...
/** Taller con cupo y modalidad. Lista en memoria para pruebas. */
...
public class Taller extends Evento implements IEventoConCupo {
    ...
    @Enumerated(EnumType.STRING) @Column(name = "modalidad") private Modalidad modalidad;
    ...
    @Override public void desinscribirParticipante(Persona persona) { 
    ...
    public void setCupoMaximo(int v) {
        ...
        if (v < participantes.size()) throw new IllegalStateException("Cupo menor a inscriptos");
        this.cupoMaximo = v;
    }
    @Override 
    ...
    public void setModalidad(Modalidad m) { if (m == null) throw new IllegalArgumentException("Modalidad nula"); this.modalidad = m; }
    ...
    public Persona getInstructor() {
        return obtenerResponsables(TipoRol.INSTRUCTOR).stream().findFirst().orElse(null);
    }
